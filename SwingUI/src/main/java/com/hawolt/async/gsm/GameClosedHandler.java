    ...
    public void onStateChange(State state) {
        ...
        try {
            ...
            if (dead) {
                ...
                JSONObject info = swiftrift.getLeagueClient().getLedge().getGameServiceMessage().getCurrentGameInformation();
                ...
                QueueWindow window = swiftrift.getLayoutManager().getQueue();
                window.getAvailableLobbies().forEach(lobby -> lobby.toggleButtonState(false, true));
            }
        } catch (IOException e) {
            Logger.error(e);
        }
