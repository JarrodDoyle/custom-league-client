...
public class JWT {
    ...
    public JWT(String plain) {
        ...
        this.headerBytes = Base64.getDecoder().decode(compartments[0]);
        this.payloadBytes = Base64.getDecoder().decode(compartments[1]);
        this.signatureBytes = Base64.getDecoder().decode(compartments[2]);
        this.header = new JSONObject(new String(headerBytes, StandardCharsets.UTF_8));
        this.payload = new JSONObject(new String(headerBytes, StandardCharsets.UTF_8));
    }

    public boolean isExpired() {
        return System.currentTimeMillis() >= getIssuedAt();
    }
